...
export class Uploader {
    ...
    //         // Ensure fileId and fileKey are available before proceeding
    //         if (this.fileId && this.fileKey) {
    //             // Create a new XMLHttpRequest for the current part
    //             // PartNumber is 1-based, so adjust to 0-based index
    //             const xhr = (this.activeConnections[part.PartNumber - 1] = new XMLHttpRequest());

    //             // Notify that the chunk upload has started
    //             sendChunkStarted();

    //             // Bind the progress listener to handle upload progress
    //             const progressListener = this.handleProgress.bind(this, part.PartNumber - 1);

    //             // Add event listeners for progress, error, abort, and loadend events
    //             xhr.upload.addEventListener("progress", progressListener);
    //             xhr.addEventListener("error", progressListener);
    //             xhr.addEventListener("abort", progressListener);
    //             xhr.addEventListener("loadend", progressListener);

    //             // Open the request with the PUT method and the pre-signed URL
    ...
    //                     // Retrieve the ETag parameter from the HTTP headers
    //                     const ETag = xhr.getResponseHeader("ETag");

    //                     if (ETag) {
    //                         // Create an object for the uploaded part
    //                         const uploadedPart = {
    //                             PartNumber: part.PartNumber,
    //                             // Remove the enclosing quotation marks from the raw ETag
    //                             ETag: ETag.replaceAll('"', ""),
    //                         };

    //                         // Add the uploaded part to the list of uploaded parts
    //                         this.uploadedParts.push(uploadedPart);

    //                         // Resolve the promise with the status code
    //                         resolve(xhr.status);
    //                         // Remove the active connection for the current part
    ...
    //             xhr.onabort = () => {
    //                 // Reject the promise with an error indicating the upload was canceled
    //                 reject(new Error("Upload canceled by user"));
    //                 // Remove the active connection for the current part
    //                 delete this.activeConnections[part.PartNumber - 1];
    //             };

    //             xhr.setRequestHeader('Access-Control-Allow-Origin', '*'); // Set the header
